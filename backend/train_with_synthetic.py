...
def create_synthetic_contracts(num_contracts=17):  # 3 real + 17 synthetic = 20 total
    """Create synthetic contracts in the database"""
    ...
    vendors = ["IT Partners LLC", "Consulting Group Inc", "Service Experts Corp", "Advisory Team Ltd"]
    
    print(f"Creating {num_contracts} synthetic contracts...")
    
    for i in range(num_contracts):
        # Random risk level distribution: 40% LOW, 35% MEDIUM, 25% HIGH
        rand = random.random()
        if rand < 0.4:
            risk_level = "LOW"
        ...
        # Create extracted clauses (simulated NLP output)
        extracted_clauses = {
        ...
        entities = {
            ...
            "organizations": [random.choice(clients), random.choice(vendors)],
            "locations": ["California", "San Francisco", "New York", "Delaware"]
        }
        
        # Generate summary
        summary = f"{risk_level} risk contract between {random.choice(clients)} and {random.choice(vendors)}. " \
...
def main():
    ...
    # Step 1: Check current contracts
    db = SessionLocal()
    ...
    # Step 2: Add synthetic contracts if needed
    if current_count < 20:
        needed = 20 - current_count
        create_synthetic_contracts(needed)
    else:
